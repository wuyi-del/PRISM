#!/bin/bash
# ============================================================
#  E2: P1-3b — τ 消融 B: τ = 1.0 (固定，≈S1 行为)
#
#  目的：验证 τ_end=1.0 时是否退化到接近 S1(Static PPL) 的行为
#  预期：τ=1.0 ≈ 无区分度加权，应接近 S1 结果(AIME24~56.56%)
#
#  与 S2 基线的唯一差异：rgopd_ppl_tau=1.0 (vs S2的5.0)
#  与 S1 的关系: S1 用 τ=1.0 但无 ppl_weighted_error 的动态特性
# ============================================================
set -x
...
python3 -m verl.trainer.main_ppo \
        ...
        # ★★★ P1-3b 关键差异: τ=1.0 (比 S2 的 5.0 更低，接近均匀加权)
        #     验证: 低终点 τ 是否导致退化到 S1 水平？
        actor_rollout_ref.actor.policy_loss.rgopd_ppl_tau=1.0 \
        # Token gate 关闭
        ...
        trainer.experiment_name='tau_ablation_B_tau1_0_30Bto4B' \
        trainer.n_gpus_per_node=8 \
        trainer.nnodes=1 \
        trainer.save_freq=50 \
        trainer.default_local_dir=/G-OPD-checkpoints/tau-ablation-B-tau1-E2 \
