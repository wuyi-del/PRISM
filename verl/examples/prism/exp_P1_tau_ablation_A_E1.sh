#!/bin/bash
# ============================================================
#  E1: P1-3a — τ 消融 A: τ = 2.0 (固定)
#
#  目的：验证 τ_start=2.0 是否足够（比S2的5.0更低）
#  预期：τ=2.0 可能保护期不足，效果介于 S1(τ=1.0) 和 S2(τ=5.0) 之间
#
#  与 S2 基线的唯一差异：rgopd_ppl_tau=2.0 (vs S2的5.0)
# ============================================================
set -x
...
python3 -m verl.trainer.main_ppo \
        ...
        # ★★★ P1-3a 关键差异: τ=2.0 (比 S2 的 5.0 更低)
        #     验证: 低起点 τ 是否仍能提供足够的课程保护？
        actor_rollout_ref.actor.policy_loss.rgopd_ppl_tau=2.0 \
        # Token gate 关闭
        ...
        trainer.experiment_name='tau_ablation_A_tau2_0_30Bto4B' \
        trainer.n_gpus_per_node=8 \
        trainer.nnodes=1 \
        trainer.save_freq=50 \
        trainer.default_local_dir=/G-OPD-checkpoints/tau-ablation-A-tau2-E1 \
