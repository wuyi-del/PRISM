#!/bin/bash
# ============================================================
#  R10: P2-7 — S2-filter (PPL 过滤极端退化轨迹)
#
#  目的：验证过滤掉 PPL 异常高的退化轨迹后是否更稳定
#  设计：只对 PPL > mean + 1*std 的错误轨迹做差异化加权
#        其余"合理错误"保持均匀权重 lw
#
#  与 S2 基线差异:
#    rgopd_ppl_filter_sigma=1.0 (新增, S2基线无此参数)
#    其余完全同 S2: λw=1.0, τ=5.0, λr=0.2
# ============================================================
set -x
...
python3 -m verl.trainer.main_ppo \
  ...
  # ⭐ 新增: PPL 过滤阈值
  actor_rollout_ref.actor.policy_loss.rgopd_ppl_filter_sigma=1.0 \
  ...
  trainer.experiment_name=P2-S2filter_PPL_30Bto4B_R10 \
  trainer.n_gpus_per_node=8 \
  trainer.nnodes=1 \
  trainer.save_freq=20 \
  trainer.default_local_dir=/workspace/G-OPD/results/P2-S2filter-R10 \
