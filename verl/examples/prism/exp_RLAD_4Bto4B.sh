#!/bin/bash
# ============================================================
#  RLAD (Trust-Region-Ratio Adaptive Distillation) 4B→4B
#
#  论文：RLAD (Chen et al., 2025)
#  核心改动：A_t = A_grpo + alpha * clip(ρ_t, 1/c, c) * log(π_teacher / π_ref_base)
#    - ρ_t = π_student / π_ref = exp(log_prob_student - log_prob_ref)
#    - clip range: [1/c, c] with c=1.5
#    - 加法融合（非乘法）: GRPO 信号 + 有 trust-region 约束的 teacher 信号
#
#  公平对比配置（与 ExOPD Exp 1d / EOPD 完全一致）：
#    - Teacher: Qwen3-4B-RL-Math (同 ExOPD)
#    - Student: Qwen3-4B-Non-Thinking (同 ExOPD)
#    - T=1.0, Batch=1024, LR=1e-5, 50 steps
#    - rlad_alpha=1.0, rlad_c=1.5 (论文默认值)
# ============================================================
set -x
...
python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='RLAD_4Bto4B_T1.0_alpha1.0_c1.5' \
        trainer.n_gpus_per_node=8 \
        trainer.nnodes=1 \
        trainer.save_freq=20 \
        trainer.test_freq=10 \
        trainer.default_local_dir=/G-OPD-checkpoints/RLAD-4Bto4B \
